...
| 技术 | 特点 | 适用 |
|------|------|------|
| **固定字符 + overlap**（如 `RecursiveCharacterTextSplitter`） | 实现简单、LangChain 内置 | v1 默认 |
| **按页切** | 页码与引用对齐容易 | 法规/审计式引用强依赖页码时 |
| **Semantic chunking**（按语义边界切） | 块内主题更纯 | 检索质量不稳时进阶尝试 |
| **结构感知**（按目录/标题） | 章节边界清晰 | 若解析能稳定抽出标题层级 |

**选择建议**：**递归字符切分 + overlap（如 500～1000 字、10%～15% overlap）** 起步；元数据里**强制带 `page`**（从 PDF 映射）。再针对 benchmark 调 `chunk_size`。
...
| 类型 | 含义 | 常见做法 |
|------|------|----------|
| **检索** | 相关问题能否召回含答案的 chunk | 人工建 20～100 条 `(问题, 招股书标识)`，标出**应命中的页码或关键句**；算 Recall@k、MRR。 |
| **忠实度（Faithfulness）** | 回答句是否被检索上下文支持 | 规则：强制脚注；或 LLM-as-judge 判断「claim 是否被 context 蕴含」（需防评委偏差）。 |
| **完整性（报告）** | 模板章节是否覆盖 | Schema / 检查清单：必填小节是否非空；「未披露」是否显式写出。 |
